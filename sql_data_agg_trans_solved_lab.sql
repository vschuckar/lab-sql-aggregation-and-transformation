...
-- 1.2. 
SELECT TIME_FORMAT(SEC_TO_TIME(AVG(length) * 60), '%H:%i') AS avg_movie_duration
FROM sakila.film;
...
*/
SELECT concat(first_name, last_name, left(email,3)) AS 'customer_mkt_campaign' 
...
*/
-- 1.1 
SELECT COUNT(DISTINCT title) AS total_num_films
FROM sakila.film;
...
SELECT DISTINCT last_name AS not_repeated_last_name
FROM sakila.actor;